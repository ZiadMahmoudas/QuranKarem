...
function savePosition(){
  try{
    if(audio.duration > 0) {
      localStorage.setItem(LS_POS,JSON.stringify({surah:currentSurah, time:Math.floor(audio.currentTime)}));
    }
  }catch(e){}
}
...
async function loadSurah(n, autoplay=false, startSec=0){
  currentSurah=n; activeVerseIdx=-1; verses=[];
  ...
  try {
    ...
    verses = allQuranText.filter(v => v.verse_key.startsWith(n + ':')).map((v, i) => ({
      text: v.text_uthmani,
      numberInSurah: i + 1
    }));
...
audio.addEventListener("timeupdate",()=>{
  ...
  const approxIdx = Math.min(verses.length - 1, Math.floor((ct / dur) * verses.length));
  if (activeVerseIdx !== approxIdx) {
      ...
      document.getElementById("playerVerseInfo").textContent=`الآية ${approxIdx+1} من ${verses.length}`;
  }
...
function showResumeToast(pos){
  ...
  document.getElementById("toastSub").textContent="توقفت عند "+ fmt(pos.time);
  const toast=document.getElementById("resumeToast");
  toast.classList.add("show");
  document.getElementById("toastResume").onclick=()=>{toast.classList.remove("show");loadSurah(pos.surah,false,pos.time);};
...
window.addEventListener("beforeunload",()=>{if(isPlaying||audio.currentTime>0)savePosition();});
