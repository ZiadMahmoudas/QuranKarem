...
// ── State ──
let currentSurah=1, verses=[], isPlaying=false, autoNext=true, muted=false, activeVerseIdx=-1, saveTimer=null;
let totalSurahDuration = 0; // متغير لحفظ مدة السورة كلها
const LS_POS="qr_position", LS_DONE="qr_completed";
let savedPos={}, completedSurahs=new Set();
const audio = document.getElementById("audioEl");
const metaAudio = document.getElementById("metaAudio"); // مشغل الخفاء لحساب الوقت
let allQuranText = null; 

// ── تنسيق الوقت (بيدعم ساعات، دقايق، ثواني) ──
function fmt(s){
...
// ── تحميل السورة (آية بآية للتظليل + استخراج الوقت الكلي في الخفاء) ──
async function loadSurah(n, autoplay=false, startAyah=0){
  currentSurah=n; activeVerseIdx=-1; verses=[]; totalSurahDuration=0;
  updateSidebarActive(n);
  const s=SURAHS[n-1];
  
  // تظبيط واجهة المستخدم بدون تكرار
  ...
  document.getElementById("quranContent").innerHTML=`<div class="loading"><p>جاري التحميل...</p><div class="loading-dots"><span class="loading-dot"></span><span class="loading-dot"></span><span class="loading-dot"></span></div></div>`;
  ...
  const readerId = selectedOption.value; // ده عشان نجيب الآيات
  const fullAudioBaseUrl = selectedOption.getAttribute("data-full"); // ده عشان نجيب الوقت الإجمالي للسورة

  // استخراج الوقت الإجمالي للسورة في الخلفية
  metaAudio.src = fullAudioBaseUrl + pad(n) + ".mp3";
  metaAudio.onloadedmetadata = () => {
      totalSurahDuration = metaAudio.duration;
      document.getElementById("timeDuration").textContent = fmt(totalSurahDuration);
  };

  try {
    if (!allQuranText) {
      const textRes = await fetch(`https://api.quran.com/api/v4/quran/verses/uthmani`);
      const textData = await textRes.json();
      allQuranText = textData.verses;
    }

    const chapterVerses = allQuranText.filter(v => v.verse_key.startsWith(n + ':'));
    const audioRes = await fetch(`https://api.quran.com/api/v4/recitations/${readerId}/by_chapter/${n}`);
    const audioData = await audioRes.json();

    verses = chapterVerses.map((verseItem, index) => {
      const audioObj = audioData.audio_files[index];
      return {
        text: verseItem.text_uthmani,
        numberInSurah: index + 1,
        audio: audioObj ? "https://verses.quran.com/" + audioObj.url : ""
      };
    });
    ...
    document.getElementById("quranContent").innerHTML=`<div class="loading"><p>⚠️ تعذر تحميل السورة، تأكد من اتصال الإنترنت.</p></div>`;
  }
}
...
// ── تشغيل الآية بدقة (Word by Word Sync Effect) ──
...
audio.addEventListener("ended",()=>{
  if(activeVerseIdx < verses.length - 1) {
    ...
    // الانتقال التلقائي للسورة اللي بعدها
...
// ── تحديث العداد الزمني للسورة بالكامل ──
audio.addEventListener("timeupdate",()=>{
  ...
  // حساب النسبة المئوية للسورة كلها بناءً على الآيات ووقت الآية الحالية
  const currentVerseProgress = ct / dur;
  const overallProgress = (activeVerseIdx + currentVerseProgress) / totalVerses;
  
  document.getElementById("progFill").style.width = (overallProgress * 100) + "%";
  
  // حساب الوقت المنقضي الفعلي وعرضه
  if (totalSurahDuration > 0) {
      const estimatedElapsed = overallProgress * totalSurahDuration;
      document.getElementById("timeElapsed").textContent = fmt(estimatedElapsed);
  }
  
  if(!saveTimer){saveTimer=setTimeout(()=>{savePosition();saveTimer=null;},5000);}
});
...
// ضغطة شريط التقدم بتوديك للآية اللي في المكان ده بالظبط
document.getElementById("progTrack").addEventListener("click",e=>{
  if(!verses.length || totalSurahDuration === 0) return;
  const rect = e.currentTarget.getBoundingClientRect();
  const pct = (e.clientX - rect.left) / rect.width;
  const targetAyah = Math.min(verses.length - 1, Math.floor(pct * verses.length));
  playAyah(targetAyah, true);
});
...
document.getElementById("prevBtn").onclick=()=>{if(currentSurah>1)loadSurah(currentSurah-1,isPlaying);};
document.getElementById("nextBtn").onclick=()=>{if(currentSurah<114)loadSurah(currentSurah+1,isPlaying);};
document.getElementById("volSlider").oninput=e=>{
  audio.volume=+e.target.value;muted=audio.volume===0;
...
document.getElementById("autoToggle").onclick=()=>{
  autoNext=!autoNext;document.getElementById("autoToggle").classList.toggle("on",autoNext);
};
...
document.getElementById("toggleSidebar").onclick=()=>{
  ...
  if(window.innerWidth<=820){sb.classList.toggle("open",sidebarOpen);}
  else{sb.classList.toggle("hidden",!sidebarOpen);mn.classList.toggle("full",!sidebarOpen);}
};
if(window.innerWidth<=820){document.getElementById("sidebar").classList.remove("open");document.getElementById("main").classList.add("full");}
...
window.addEventListener("beforeunload",()=>{if(isPlaying||activeVerseIdx>=0)savePosition();});
...
if(savedPos&&savedPos.surah){
  loadSurah(savedPos.surah,false).then(()=>setTimeout(()=>showResumeToast(savedPos),700)).catch(()=>loadSurah(1));
}else{
  loadSurah(1,false);
}